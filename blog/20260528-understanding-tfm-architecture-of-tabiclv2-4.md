...
Start with standard scaled dot-product attention. A query \(q\in\mathbb{R}^{d_\text{head}}\) is compared with keys \(k_1,\ldots,k_n\in\mathbb{R}^{d_\text{head}}\). The unnormalized attention logit for key \(j\) is
$$
...
$$
The output is the weighted average \(\sum_{j=1}^n a_jv_j\), where \(v_j\) is the value vector associated with key \(j\). QASSMax does not replace this attention mechanism. It changes the logits before the softmax by rescaling the query.

If the query is rescaled as \(\bar{q}=\alpha q\), then
$$
...
$$
Writing \(\alpha=1/\tau\), we get
$$
\text{softmax}_\tau(z)=\text{softmax}(\alpha z).
$$
Lower temperature, or larger \(\alpha\), sharpens attention; higher temperature, or smaller \(\alpha\), spreads attention more broadly. The relative odds between keys \(j\) and \(\ell\) become
$$
\frac{a_j}{a_\ell}
=
\frac{\exp(\alpha z_j)}{\exp(\alpha z_\ell)}
=
...
The need for scalable softmax comes from attention fading. Suppose one relevant key has logit \(z_\star\), and the other \(n-1\) distractors have a lower logit \(z_0\). Let \(\Delta=z_\star-z_0>0\). Under ordinary softmax,
$$
a_\star
=
...
$$
This expression shows why \(\log n\) matters: to keep the relevant token visible as \(n\) grows, the relevant logit gap must effectively grow on the order of \(\log n\).
...
$$
Here \(\odot\) denotes element-wise multiplication. For \(H\) attention heads, \(\text{MLP}_\text{base}: \mathbb{R}\rightarrow\mathbb{R}^{H\times d_\text{head}}\) and \(\text{MLP}_\text{gate}:\mathbb{R}^{d_\text{head}} \rightarrow \mathbb{R}^{d_\text{head}}\) are two-layer MLPs with 64 hidden neurons and GELU activation.
...
$$
where \(H_\alpha\) is a Tsallis-entropy-style regularizer. The relevant lesson for QASSMax is not sparsity itself, since QASSMax still uses softmax. The relevant lesson is that attention normalization can benefit from a learned function of context length, such as the ASEntmax form
$$
\delta+\beta(\log n)^\gamma.
$$
QASSMax generalizes this idea through \(\text{MLP}_\text{base}(\log n)\).
...
Query-aware scalable softmax modifies attention logits by rescaling queries with both a context-length-dependent base term and a bounded query-dependent gate. This helps TabICLv2 keep attention sharp in long contexts while allowing different queries and latent dimensions to adjust the amount of scaling they need. The next post covers quantile predictions for regression, the regression strategy TabICLv2 uses to model predictive uncertainty without discretizing the target into classification bins.
